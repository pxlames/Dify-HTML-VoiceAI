...
const server = http.createServer(async (req, res) => {
    try {
        ...
        // 特殊处理：如果是音频文件且在static目录中不存在，尝试从根目录查找
        if (!fsSync.existsSync(filePath) && (pathname.endsWith('.mp3') || pathname.endsWith('.wav') || pathname.endsWith('.ogg'))) {
            const rootFilePath = path.join(__dirname, pathname);
            if (fsSync.existsSync(rootFilePath)) {
                filePath = rootFilePath;
                resolvedPath = path.resolve(filePath);
                staticDir = __dirname; // 更新staticDir为根目录
            }
        }
        
        // 禁止访问允许目录外的资源（static目录或根目录）
        const allowedDirs = [path.join(__dirname, 'static'), __dirname];
